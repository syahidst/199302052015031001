...
return window.open(url, title, 'toolbar=yes, location=no,directories=no, status=no, menubar=no, scrollbars=no, resizable=yes');
    ...
    <!--
								  ...
								  $no=1;
								  $prosedur_perkara=mysql_query("SELECT * FROM kelola_konten WHERE jenis_konten='prosedur_perkara' ");
									while($r=mysql_fetch_array($prosedur_perkara)){

								  ?>
								  <li class="collection-item">
                                      <a href="home.php?page=detail-prosedur-perkara&judul=<?php echo $r['id_konten'].'-'.judul_seo($r['judul_konten']);?>" class="waves-effect"><span><?php echo $no.'. '.$r['judul_konten'];?></span><i class="arrow mdi mdi-chevron-right"></i></a>
                                  </li>
									<?php
